...
//
// Every rule also carries a `category` — one of 8 broader buckets (see
// docs/PromptCoach_Direction_and_Roadmap.pdf) used for the per-user skill
// dashboard (profile.js) so 20 specific tips roll up into a handful of
// trends ("you're strong at giving context, weaker at specifying format")
// without losing the specific, actionable tip itself.
// =============================================================================

(function () {
  // classify() — generative/analytical ask vs. a discrete factual query.
  // Gates whether refinement-style coaching (iteration nudges, auto AI
  // critique) applies at all: no point coaching "what's 7*8".
  function classify(rawText) {
    var text = (rawText || "").trim();
    if (!text) return "factual";
    ...
    var openEndedWords = ["essay", "report", "story", "poem", "code", "script", "analysis",
      "summary", "outline", "explain", "compare", "design", "email", "letter",
      "review", "brainstorm", "ideas"];
    var l = text.toLowerCase();
    var hasOpenEnded = openEndedWords.some(function (w) { return l.indexOf(w) >= 0; });

    if (isTask || hasOpenEnded) return "generative";
    if (isQ && wc <= 12) return "factual";
    return wc <= 8 ? "factual" : "generative";
  }
  ...
  function evaluateAll(rawText) {
    ...
    if (sensitive) all.push({ prio: 1, id: "sensitive", category: "safety",
    ...
    if (isQ && !has(["source", "cite", "citation", "reference", "according to"]))
      all.push({ prio: 2, id: "grounding", category: "accuracyGrounding",
    ...
    if (has(codeWords) && !has(stackWords))
      all.push({ prio: 3.5, id: "techConstraints", category: "missingConstraints",
    ...
    if (isEditTask)
      all.push({ prio: 2.5, id: "editInAI", category: "iterationMindset",
    ...
    // 5 — Let it ask first
    if (isTask || wc < 8)
      all.push({ prio: 3, id: "clarify", category: "vagueAsk",
    ...
    // 3 — Context
    if (wc < 6)
      all.push({ prio: 4, id: "context", category: "missingContext",
    ...
    if (isTask && has(planWords) && !has(constraintWords))
      all.push({ prio: 4.5, id: "planConstraints", category: "missingConstraints",
    ...
    if (!has(["bullet", "concise", "short", "brief", "one sentence", "paragraph",
      ...
      all.push({ prio: 5, id: "format", category: "outputFormat",
    ...
    if (has(comparisonWords) && !has(structureWords))
      all.push({ prio: 5.5, id: "structure", category: "outputFormat",
    ...
    if (isTask && !has(["for ", "audience", "tone", "formal", "casual", "professional",
      ...
      all.push({ prio: 6, id: "audience", category: "missingContext",
    ...
    if (has(expertWords) && !has(personaWords))
      all.push({ prio: 6.5, id: "persona", category: "missingConstraints",
    ...
    if (isTask && has(["good", "great", "best", "nice", "better", "perfect"]))
      all.push({ prio: 7, id: "criteria", category: "vagueAsk",
    ...
    if (isTask && wc >= 10 && !has(negWords))
      all.push({ prio: 7.5, id: "negative", category: "missingConstraints",
    ...
    if (isTask && wc < 25 && !has(["example", "e.g", "for instance", "such as", "like this"]))
      all.push({ prio: 8, id: "example", category: "missingContext",
    ...
    if (isTask && wc >= 8 && wc < 40 && !has(optionWords))
      all.push({ prio: 8.5, id: "options", category: "iterationMindset",
    ...
    if (has(["latest", "recent", "current", "today", "this year", "newest", "right now", "2024", "2025", "2026"]))
      all.push({ prio: 9, id: "recency", category: "accuracyGrounding",
    ...
    if (has(reasoningWords) && !has(showWorkWords))
      all.push({ prio: 9.5, id: "stepByStep", category: "outputFormat",
    ...
    if (isTask && wc >= 15 && !has(selfCheckWords))
      all.push({ prio: 10.5, id: "selfCheck", category: "accuracyGrounding",
  ...
  var api = { analyze: analyze, evaluateAll: evaluateAll, classify: classify };
